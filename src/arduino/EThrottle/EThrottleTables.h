...
#define TABLE_SIZE_TEST_PAGE 128
...
#define FIELD_OFFSET_TEST_PAGE(FIELD_NAME) (offsetof(TestPage_T, FIELD_NAME))
...
const uint8_t  MAX_IDLE_ADDER_AUTHORITY    = 20    ;
const uint8_t  MAX_UART_CMD_LEN            = 8     ;
const uint8_t  TEST_PLAYBACK_CURVE_N_BINS  = 12    ;
...
struct Status0Bits_T {
    ...
    // 1 if MCU hasn't received, or is receiving erroneous Megasquirt realtime broadcast data
    // range: [0.000000,1.000000]
    uint8_t msqRT_BCastListenFault : 1;
...
struct Status1Bits_T {
    // 1 if throttle controller is in test mode
    // range: [0.000000,1.000000]
    uint8_t testModeEnabled : 1;

    // 1 if the playback curve contains illegal data points (x-axis points aren't ascending)
    // range: [0.000000,1.000000]
    uint8_t testModeBadCurve : 1;
...
struct ThrottleOutVars_T {
    ...
    // Motor driver fault counter (wraps)
    // range: [0.000000,255.000000]
    uint8_t driverFaultCount;

    // TPS delta over time
    // range: [-1500.000000,1500.000000]
    // units: %/s
    int16_t TPSdot;
};
...
struct OutPC_T {
    ...
    // auto-added member to pad this table struct to 128 bytes
    // range: [0.000000,1.000000]
    uint8_t __tablePad[84];
};
...
struct Page2_T {
    ...
    // auto-added member to pad this table struct to 128 bytes
    // range: [0.000000,1.000000]
    uint8_t __tablePad[121];
};
static_assert(sizeof(Page2_T) == 128);
static_assert(sizeof(Page2_T) <= MEGA_CAN_EXT_MAX_FLASH_TABLE_SIZE);

struct TestPage_T {
    // Throttle setpoint while in 'single setpoint' test mode.
    // scale: 0.010000; translate: 0.000000
    // range: [0.000000,100.000000]
    // units: %
    uint16_t testSetpoint;

    // Number of times to playback the test curve (0 = indefinite)
    // range: [0.000000,255.000000]
    uint8_t testPlaybackCount;

    // range: [0.000000,65535.000000]
    // units: ms
    uint16_t testPlaybackCurve_xBins[TEST_PLAYBACK_CURVE_N_BINS];

    // range: [0.000000,100.000000]
    // units: %
    uint8_t testPlaybackCurve_yBins[TEST_PLAYBACK_CURVE_N_BINS];

    // auto-added member to pad this table struct to 128 bytes
    // range: [0.000000,1.000000]
    uint8_t __tablePad[89];
};
static_assert(sizeof(TestPage_T) == 128);
static_assert(sizeof(TestPage_T) <= MEGA_CAN_EXT_MAX_FLASH_TABLE_SIZE);

extern OutPC_T outPC;
extern TestPage_T testPage;
...
static const MegaCAN::TableDescriptor_t TABLES[NUM_TABLES] = {
    ...
    {&testPage        , sizeof(TestPage_T), MegaCAN::TableType_E::eRam  , -1                          },
    {nullptr          , MAX_UART_CMD_LEN  , MegaCAN::TableType_E::eRam  , -1                          },
};